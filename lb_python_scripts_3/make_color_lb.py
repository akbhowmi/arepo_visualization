...
import Image, ImageColor, ImageDraw, ImageFont, pyfits, os.path, types, pdb
from numpy import *
...
def make_image(image,return_jpeg=True,plotlabel='',plotlabel2='',plotscale=False,**kwargs):
    """
...
def write_color(file, hdu, output_file, overwrite=False,
                imagefunc=make_image, plotlabel='', plotscale=False, **kwargs):
    """
...
"""
    ...
    if not overwrite and os.path.exists(output_file):
        ...
        img=imagefunc(transpose(pyfits.open(file)[hdu].data, axes=(1,2,0)), 
                      plotlabel=plotlabel, plotscale=plotscale, **kwargs)
